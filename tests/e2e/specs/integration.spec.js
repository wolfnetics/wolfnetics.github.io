describe('Integration tests', () => {
    ...
    it('has the list of solutions', () => {
        ...
        cy.get('.solutions').each((item) => {
            ...
            cy.get('@solution').then(($el) => {
                if ($el.find('a.stretched-link').length) {
                    ...
                    // To properly wait for the modal to close, we have to pipe the click and continue clicking until
                    //   it works. In some test runs, the runner will try to click 17 times on the button!
                    //   More info here: https://www.cypress.io/blog/2019/01/22/when-can-the-test-click/
                    const click = $el => $el.click();
                    cy.get('@modal').find('button.btn').pipe(click).should('not.be.visible').then(() => {
                        cy.get('@modal').should('not.be.visible');
                    });
                }
            });
    ...
    it('has the icon links', () => {
        cy.get('a.icon-link').each((item) => {
    ...
    it('has the disclaimer text', () => {
        cy.get('p.text-muted small').should('exist');
    });

    it('has the copyright text', () => {
        cy.get('p').contains(`© ${new Date().getFullYear()}`);
    });
});
